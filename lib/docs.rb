puts "Loading lib/docs.rb"
puts "Ruby version: #{RUBY_VERSION}"
puts "Bundler version: #{Bundler::VERSION}"

puts "Load path at start of lib/docs.rb:"
puts $LOAD_PATH

puts "Loaded features at start of lib/docs.rb:"
puts $LOADED_FEATURES.grep(/html/)

begin
  require 'bundler/setup'
  puts "Bundler setup successful"
rescue => e
  puts "Error in Bundler setup: #{e.message}"
end

begin
  Bundler.require :default, :docs
  require 'html-pipeline'
  puts "Bundler.require successful"
rescue => e
  puts "Error in Bundler.require: #{e.message}"
end

begin
  Bundler.locked_gems.specs.each do |gem|
    puts "Loaded: #{gem.name} (#{gem.version})"
  end
rescue => e
  puts "Error in listing gems: #{e.message}"
end

puts "HTML::Pipeline loaded after Bundler.require: #{defined?(HTML::Pipeline)}"

begin
  require 'active_support'
  require 'active_support/core_ext'
  puts "ActiveSupport loaded successfully"
rescue => e
  puts "Error loading ActiveSupport: #{e.message}"
end
